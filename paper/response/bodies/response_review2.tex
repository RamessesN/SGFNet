...
We sincerely thank the reviewer for raising this important point. We fully agree that class imbalance is indeed present in our experimental results, and we have carefully analyzed its underlying causes and added a corresponding discussion to the revised manuscript.
...
\textbf{(3) Regarding our preliminary exploration and future work:}

To further investigate whether the underperformance on minority classes is primarily attributable to class imbalance, we conducted a preliminary exploration using two common imbalance-mitigation strategies: focal loss and minority-class oversampling. We observed a modest improvement on the affected minority classes under these strategies, suggesting that class imbalance is indeed a contributing factor, though the spectral ambiguity discussed above likely also plays a substantial role and is not fully resolved by loss re-weighting or resampling alone. As a systematic study of imbalance-handling techniques is beyond the scope of this letter, we have explicitly identified this as an important direction for future work in the Conclusion of the revised manuscript.
...
The results confirm that SGFNet exhibits substantially smaller performance degradation as the shift magnitude increases compared to DFFNet and MSFMamba, verifying that FMFB effectively addresses the issue of spatial misalignment through frequency-domain modulation. This experiment is presented in detail in our response to Comment~2 of Reviewer~1, and the corresponding results have been added to Table [V] in Section III-E of the revised manuscript.

\begin{table}[h]
  \centering
  \caption*{TABLE V Comparison of OA (\%) under different degrees of artificial spatial shifts on the Houston2018 dataset.}
...
We have added the following clarification on the mechanism of FMFB to \textbf{Section II-C (Frequency Modulated Fusion Block)} of the revised manuscript. The specific added text is as follows:
...
We have added the following clarification to \textbf{Section II-B (Semantic Mixing Convolution Block)} of the revised manuscript, as part of the same paragraph contrasting SMCB with prior dynamic convolution methods (added in response to Comment~1 of Reviewer~1). The specific added text is as follows:
...
We hope this explanation effectively clarifies the physical meaning underlying SMCB's long-range dependency modeling capability. All corresponding changes have been highlighted in red in the updated manuscript for your review. Thank you again for guiding us to improve our manuscript.
...
``\textbf{\textcolor{red}{Sensitivity of the Dynamic Kernel Size in SMCB.}} \textcolor{red}{The dynamic kernel size $K$ in SMCB determines the number of semantic sampling locations used to generate the dynamic convolution kernel, and may therefore affect the classification performance. We evaluate SGFNet with $K \in \{5, 7, 9, 11\}$ on both the Augsburg and Houston2018 datasets, and the results are shown in Table~II. The OA on both datasets peaks at $K=7$ (92.58\% on Augsburg and 94.10\% on Houston2018), and gradually declines as $K$ deviates from this value in either direction. A smaller $K$ provides insufficient semantic sampling locations for effective contextual modeling, while a larger $K$ introduces potentially redundant or noisy contextual information along with increased computational overhead. Based on this analysis, we set $K=7$ in our experiments, as it achieves the best trade-off between classification accuracy and computational overhead.}''

\begin{table}[h]
  \centering
  \caption*{\textcolor{red}{TABLE II OA (\%) of SGFNet under different dynamic kernel sizes $K$ in SMCB.}}
  \resizebox{0.65\columnwidth}{!}{
  \begin{tabular}{lcccc}
  \toprule
  $K$ & 5 & 7 & 9 & 11 \\
